...
test("active activation retry fails closed after hostname claim release", async t => {
  const f = fixture(t);
  assert.equal((await activatePublisherCommercially(f.database, input())).status, 200);
  f.sqlite.exec(`
    ...
    WHERE domain_id='d'
  `);
  assert.equal((await activatePublisherCommercially(f.database, input())).status, 409);
});
